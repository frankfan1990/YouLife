...
- (MyPageControl *)pageControl
{
    ...
    if (self.totalPageCount > 1) {
        if (!_pageControl) {
            ...
            CGRect pageControlFrame = CGRectMake(CGRectGetMidX(self.scrollView.frame) - 0.5 * pageControlWidth , 0.92 * CGRectGetHeight(self.scrollView.frame), pageControlWidth, normalDotImage.size.height);
...
#warning fix bug
                if(self.fetchContentViewAtIndex(tempIndex)){/*添加对象的时候必须判断改对象是否为空*/
